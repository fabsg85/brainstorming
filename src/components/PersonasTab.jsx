    ...
    <div style={{ display:"grid", gap:14 }}>
              ...
              <div style={{ padding:"16px 20px", display:"grid", gap:12 }}>
                ...
                {p.frase && (
                  <div style={{ background:"var(--surface)", border:`1px solid ${col.border}`, borderRadius:10, padding:"12px 14px" }}>
                    <div style={{ fontSize:13, color:"var(--textMid)", lineHeight:1.6, fontStyle:"italic" }}>"{p.frase}"</div>
                  </div>
                )}
                ...
                {[
                  ...
                  <div key={label} style={{ background:bg, border:`1px solid ${border}`, borderRadius:10, padding:"10px 14px" }}>
                    ...
                    <div style={{ fontSize:13, color:"var(--textMid)", lineHeight:1.5 }}>{value}</div>
                  </div>
                ))}
                ...
                {p.triggers?.length > 0 && (
                  <div>
                    <div style={{ fontSize:10, fontWeight:700, color:"var(--textMute)", textTransform:"uppercase", letterSpacing:"0.7px", marginBottom:6, fontFamily:"'Sora', sans-serif" }}>⚡ Qué lo activa a comprar</div>
      ...
      {a.personaInsight && (
        <div style={{ background:"rgba(255,181,71,0.08)", border:"1px solid rgba(255,181,71,0.2)", borderRadius:14, padding:"16px 20px" }}>
          ...
          <p style={{ margin:0, fontSize:14, color:"var(--textMid)", lineHeight:1.65, fontStyle:"italic" }}>{a.personaInsight}</p>
        </div>
      )}
    </div>
  );

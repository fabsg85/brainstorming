...
export default function PreSellTab({ ideaId, presell = [], onSave }) {
  ...
  const inputStyle = { border:"1px solid var(--surface2)", borderRadius:9, padding:"10px 12px", fontSize:13, outline:"none", color:"var(--text)", background:"var(--surface)", fontFamily:"'DM Sans', sans-serif", width:"100%", boxSizing:"border-box", transition:"border-color 0.15s" };
  ...
  return (
    <div style={{ display:"grid", gap:14 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10 }}>
        {[
          { label:"Total contactos", value:metrics.total,       color:"var(--text)",    icon:"👥" },
          ...
          <div key={m.label} style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"16px 18px", backdropFilter:"blur(8px)" }}>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--textMute)", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:7, fontFamily:"'Sora', sans-serif" }}>{m.icon} {m.label}</div>
      ...
      {metrics.mrr > 0 && (
        <div style={{ background:"linear-gradient(135deg,rgba(0,245,212,0.15),rgba(108,92,231,0.06))", border:"1px solid rgba(0,245,212,0.25)", borderRadius:14, padding:"16px 22px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontSize:10, fontWeight:700, color:"#00F5D4", textTransform:"uppercase", letterSpacing:"0.8px", fontFamily:"'Sora', sans-serif" }}>💰 Revenue pre-vendido</div>
            <div style={{ fontSize:11, color:"var(--textMute)", marginTop:3 }}>Plata real sobre la mesa antes del MVP</div>
          </div>
      ...
      {presell.length > 0 && (
        <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, padding:"16px 20px", backdropFilter:"blur(8px)" }}>
          <div style={{ fontSize:10, fontWeight:700, color:"var(--textMute)", textTransform:"uppercase", letterSpacing:"1px", marginBottom:14, fontFamily:"'Sora', sans-serif" }}>Funnel de validación</div>
            ...
            return (
              <div key={s.key} style={{ marginBottom:10 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                  <span style={{ fontSize:12, color:s.color, fontWeight:700, fontFamily:"'Sora', sans-serif" }}>{s.emoji} {s.label}</span>
      ...
      <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:14, overflow:"hidden", backdropFilter:"blur(8px)" }}>
        <div style={{ padding:"14px 18px", borderBottom:"1px solid var(--border)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ fontSize:10, fontWeight:700, color:"var(--textMute)", textTransform:"uppercase", letterSpacing:"0.8px", fontFamily:"'Sora', sans-serif" }}>Contactos · {presell.length}</div>
        ...
        {presell.length === 0 ? (
          <div style={{ textAlign:"center", padding:"40px 20px" }}>
            ...
            <div style={{ color:"var(--textMid)", fontFamily:"'Sora', sans-serif", fontWeight:600, marginBottom:4 }}>Sin contactos todavía</div>
            <div style={{ color:"var(--textMute)", fontSize:12 }}>Cargá los prospects que ya hablaste para trackear tu pre-venta</div>
          </div>
        ) : (
              ...
              <div key={contact.id || i} style={{ padding:"12px 18px", borderTop:"1px solid var(--border)", display:"flex", alignItems:"center", gap:12, transition:"background 0.1s" }}
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:3 }}>
                    <span style={{ fontWeight:700, fontSize:13, color:"var(--text)", fontFamily:"'Sora', sans-serif" }}>{contact.nombre}</span>
                    {contact.empresa && <span style={{ fontSize:11, color:"var(--textMute)" }}>· {contact.empresa}</span>}
                    {contact.rol && <span style={{ fontSize:11, color:"var(--textMute)" }}>· {contact.rol}</span>}
                  </div>
                  {contact.nota && <div style={{ fontSize:12, color:"var(--textMute)", lineHeight:1.4 }}>{contact.nota}</div>}
                </div>
                {/* Status badge */}
                <span style={{ background:s.bg, color:s.color, border:`1px solid ${s.border}`, borderRadius:99, padding:"3px 10px", fontSize:11, fontWeight:700, fontFamily:"'Sora', sans-serif", flexShrink:0 }}>
                  {s.emoji} {s.label}
                </span>
                {/* Monto */}
                {contact.monto && (
                ...
                {/* Fecha */}
                <span style={{ fontSize:11, color:"var(--textMute)", fontFamily:"monospace", flexShrink:0 }}>{contact.fecha}</span>
                {/* Actions */}
      ...
      {metrics.total >= 3 && (
        <div style={{ background: metrics.conversion >= 20 ? "rgba(0,245,212,0.08)" : metrics.conversion >= 10 ? "rgba(255,181,71,0.08)" : "rgba(255,95,122,0.08)", border: `1px solid ${metrics.conversion >= 20 ? "rgba(0,245,212,0.2)" : metrics.conversion >= 10 ? "rgba(255,181,71,0.2)" : "rgba(255,95,122,0.2)"}`, borderRadius:14, padding:"16px 20px" }}>
          <div style={{ fontSize:10, fontWeight:700, color: metrics.conversion >= 20 ? "#00F5D4" : metrics.conversion >= 10 ? "#FFB547" : "#FF5F7A", textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:6, fontFamily:"'Sora', sans-serif" }}>
            🦈 Lectura del Shark
          </div>
          <p style={{ margin:0, fontSize:14, color:"var(--textMid)", lineHeight:1.65 }}>
          ...
          <div style={{ background:"var(--bg2)", border:"1px solid var(--surface2)", borderRadius:20, width:"100%", maxWidth:460, overflow:"hidden", boxShadow:"0 40px 100px rgba(0,0,0,0.6)" }}>
            <div style={{ background:"linear-gradient(135deg,rgba(108,92,231,0.2),rgba(0,245,212,0.06))", padding:"18px 22px", borderBottom:"1px solid var(--surface2)", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                <div style={{ fontWeight:700, fontSize:15, color:"#fff", fontFamily:"'Sora', sans-serif" }}>{editIdx !== null ? "✏️ Editar contacto" : "🤝 Nuevo contacto"}</div>
                <button onClick={() => setShowForm(false)} style={{ background:"var(--surface)", border:"none", borderRadius:7, color:"var(--textMid)", width:30, height:30, cursor:"pointer" }}>✕</button>
              </div>
            </div>
            ...
            <div style={{ padding:"20px 22px", display:"grid", gap:12 }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                {[
                  ...
                  <div key={f.key} style={{ gridColumn: f.key === "nombre" ? "1/-1" : "auto" }}>
                    <div style={{ fontSize:11, fontWeight:700, color:"var(--textMute)", marginBottom:5, fontFamily:"'Sora', sans-serif", textTransform:"uppercase", letterSpacing:"0.5px" }}>{f.label}</div>
              ...
              {/* Status */}
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:"var(--textMute)", marginBottom:8, fontFamily:"'Sora', sans-serif", textTransform:"uppercase", letterSpacing:"0.5px" }}>Estado</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  {STATUS_OPTIONS.map(s => (
                    <button key={s.key} onClick={() => setForm(p => ({...p, status:s.key}))}
                      style={{ background: form.status === s.key ? s.bg : "var(--surface)", color: form.status === s.key ? s.color : "var(--textMute)", border:`1px solid ${form.status === s.key ? s.border : "var(--surface2)"}`, borderRadius:99, padding:"5px 12px", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"'Sora', sans-serif", transition:"all 0.12s" }}>
                      {s.emoji} {s.label}
                    </button>
                  ))}
                </div>
                ...
                {[
                  ...
                  <div key={f.key}>
                    ...
                    <input type={f.type||"text"} value={form[f.key]} onChange={e => setForm(p => ({...p, [f.key]:e.target.value}))}
                      placeholder={f.ph} style={{ ...inputStyle, fontFamily: f.type === "number" ? "monospace" : "'DM Sans', sans-serif" }} onFocus={focus} onBlur={blur}/>
                  </div>
                ))}
              </div>
              ...
              {/* Nota */}
              <div>
                <div style={{ fontSize:11, fontWeight:700, color:"var(--textMute)", marginBottom:5, fontFamily:"'Sora', sans-serif", textTransform:"uppercase", letterSpacing:"0.5px" }}>Nota</div>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={() => setShowForm(false)} style={{ flex:1, background:"var(--surface)", border:"1px solid var(--border)", borderRadius:10, padding:"11px", color:"var(--textMid)", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"'Sora', sans-serif" }}>Cancelar</button>
                <button onClick={submitForm} disabled={!form.nombre.trim()} style={{ flex:2, background: form.nombre.trim() ? "linear-gradient(135deg,#6C5CE7,#00F5D4)" : "var(--surface)", border:"none", borderRadius:10, padding:"11px", color: form.nombre.trim() ? "#fff" : "var(--textMute)", fontWeight:700, fontSize:13, cursor: form.nombre.trim() ? "pointer" : "not-allowed", fontFamily:"'Sora', sans-serif", boxShadow: form.nombre.trim() ? "0 0 16px rgba(108,92,231,0.4)" : "none" }}>

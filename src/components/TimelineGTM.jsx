      ...
      {PHASES.map((p,i) => (
        <div key={i} style={{ display:"flex", gap:18, marginBottom:i<2?22:0, position:"relative" }}>
          ...
          <div style={{ flex:1, background:p.bg, border:`1px solid ${p.border}`, borderRadius:12, padding:"12px 16px", backdropFilter:"blur(8px)" }}>
            <div style={{ fontSize:10, fontWeight:800, color:p.color, textTransform:"uppercase", letterSpacing:"0.8px", marginBottom:6, fontFamily:T.fontDisplay }}>
              {p.icon} {p.label}
            </div>
            <p style={{ margin:0, fontSize:13, color:T.textMid, lineHeight:1.65, fontFamily:T.font }}>
              {sentences[i]||"..."}
            </p>
          </div>
        </div>

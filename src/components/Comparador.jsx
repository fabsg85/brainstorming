  ...
  return (
    <div style={{ padding:"28px 20px", fontFamily:"'DM Sans', sans-serif" }}>
      ...
      <div style={{ marginBottom:40 }}>
        ...
        <div style={{ color:"var(--textMute)", fontSize:13, marginBottom:24 }}>{analyzed.length} analizadas · ordenadas por score del Shark</div>
        ...
        {ideas.length>1 && (
          <div style={{ background:"var(--surface)", border:"1px solid var(--border)", borderRadius:18, padding:"22px", marginBottom:28, backdropFilter:"blur(12px)", boxShadow:"0 4px 24px rgba(0,0,0,0.3)" }}>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--textMute)", textTransform:"uppercase", letterSpacing:"1px", marginBottom:18, fontFamily:"'Sora', sans-serif" }}>Comparativa de scores</div>
        ...
        <div style={{ overflowX:"auto" }}>
          <div style={{ minWidth:560 }}>
            <div style={{ display:"grid", gridTemplateColumns:"28px 1fr 56px 56px 56px 56px 56px 80px", gap:6, padding:"8px 14px", background:"var(--surface)", borderRadius:10, marginBottom:6, fontSize:10, fontWeight:700, color:"var(--textMute)", textTransform:"uppercase", letterSpacing:"0.8px", fontFamily:"'Sora', sans-serif", border:"1px solid var(--border)" }}>
              ...
              return (
                <div key={idea.id} style={{ display:"grid", gridTemplateColumns:"28px 1fr 56px 56px 56px 56px 56px 80px", gap:6, padding:"12px 14px", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:12, marginBottom:6, alignItems:"center", backdropFilter:"blur(8px)", transition:"border-color 0.2s" }}>
                  <div style={{fontWeight:800,fontSize:14}}>
                    {idx===0?"🥇":idx===1?"🥈":idx===2?"🥉":<span style={{color:"var(--textMute)",fontSize:13}}>{idx+1}</span>}
                  </div>
                  <div>
      ...
      <div style={{ color:"var(--textMute)", fontSize:13, marginBottom:20 }}>Dos ideas. Una gana.</div>
      ...
      {analyzed.length<2 ? (
        <div style={{ textAlign:"center", padding:"44px", background:"var(--surface)", border:"1px solid var(--border)", borderRadius:16, backdropFilter:"blur(8px)" }}>
          ...
          <div style={{color:"var(--textMid)",fontFamily:"'Sora', sans-serif"}}>Necesitás al menos 2 ideas analizadas</div>
        </div>
      ) : (
        <div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:22 }}>
            {[
              ...
              <select key={label} value={val||""} onChange={e=>set(Number(e.target.value))}
                style={{...selectStyle, border:`1px solid ${color}30`, color:val?"var(--text)":"var(--textMute)"}}>
                ...
                return (
                  <div key={idea.id} style={{border:`1px solid ${color}20`,borderRadius:18,overflow:"hidden",boxShadow:`0 0 40px ${color}10`,backdropFilter:"blur(12px)"}}>
                    <div style={{background:`linear-gradient(135deg,${color}18,${color}06)`,padding:"18px 20px",borderBottom:"1px solid var(--border)",position:"relative",overflow:"hidden"}}>
                      ...
                      <div style={{color:"var(--textMute)",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:5,fontFamily:"'Sora', sans-serif"}}>Idea {col===0?"A":"B"}</div>
                      ...
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
                        ...
                        {sc&&(
                          <div style={{textAlign:"right"}}>
                            ...
                            <div style={{color:"var(--textMute)",fontSize:10,marginTop:2,fontFamily:"'Sora', sans-serif"}}>{scoreLabel(sc)}</div>
                          </div>
                        )}
                      </div>
                    </div>
                    <div style={{padding:"18px 20px",background:"var(--surface)",display:"grid",gap:10}}>
                      ...
                      {ia?.veredicto&&(
                        <div style={{background:"var(--surface)",borderRadius:10,padding:"12px 14px",border:"1px solid var(--border)"}}>
                          <div style={{fontSize:10,fontWeight:700,color:"var(--textMute)",marginBottom:5,textTransform:"uppercase",letterSpacing:"0.8px",fontFamily:"'Sora', sans-serif"}}>Veredicto</div>
                          <p style={{margin:0,fontSize:13,color:"var(--textMid)",lineHeight:1.5,fontStyle:"italic"}}>"{ia.veredicto}"</p>
                        </div>
                      )}
                    </div>
                  </div>
